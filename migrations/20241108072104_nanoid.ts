...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  sql`DROP FUNCTION IF EXISTS nanoid(int, text, float);`.execute(db)
  sql`CREATE OR REPLACE FUNCTION nanoid(
  ...
  sql`DROP FUNCTION IF EXISTS nanoid_optimized(int, text, int, int);`.execute(
    db
  )
  sql`CREATE OR REPLACE FUNCTION nanoid_optimized(
